...
const getLicense = async (id: string | undefined, title: string | undefined, cwd: string) => {
  ...
  if (!id) {
    ...
    id = await askToUseFoundLicense("pyproject.toml", pyProjectToml?.tool?.poetry?.license)
  }
  
  if (!id) {
    const cargoToml = await readCargo(cwd);
    id = await askToUseFoundLicense("Cargo.toml", cargoToml?.package?.license)
  }
...
const askToUseFoundLicense = async (fileName: string, licenseId: string | undefined) => {
